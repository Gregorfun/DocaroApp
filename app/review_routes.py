...
from core.review_service import (  # noqa: E402
    DocumentStatus,
    ReviewSettings,
    finalize_document,
...
def review_queue_page():
    ...
    review_docs = [
        doc for doc in results if doc.get("review_status") in [DocumentStatus.NEEDS_REVIEW, DocumentStatus.READY]
    ]

    # Load settings für Ampel-Farben
    settings = load_review_settings(config.DATA_DIR / "settings.json")

    return render_template("review_queue.html", documents=review_docs, settings=settings)
...
def review_detail_page(file_id: str):
    ...
    return render_template("review_detail.html", doc=doc, settings=settings, canonical_suppliers=canonical_suppliers)
...
def api_review_queue():
    ...
    return jsonify({"ok": True, "count": len(review_docs), "documents": review_docs})
...
def api_correct_document(file_id: str):
    ...
    try:
        corrections_path = config.DATA_DIR / "corrections.jsonl"
    ...
    if should_finalize:
        try:
            ...
            finalize_result = finalize_document(
                doc_id=file_id, source_path=source_path, extraction_result=doc, settings=settings, mode="move"
            )
    ...
    return jsonify(
        {
            ...
            "finalized_path": (
                str(finalize_result.finalized_path) if finalize_result and finalize_result.success else None
            ),
        }
    )
...
def api_finalize_document(file_id: str):
    ...
    if doc.get("review_status") != DocumentStatus.READY:
        return jsonify({"ok": False, "error": "not_ready", "message": "Document must be READY status to finalize"}), 400
    ...
    try:
        ...
        if finalize_result.success:
            ...
            return jsonify({"ok": False, "error": "finalize_failed", "message": finalize_result.error}), 500
    except Exception as exc:
        _LOGGER.error(f"Finalize failed: {exc}")
        return jsonify({"ok": False, "error": "finalize_exception", "message": str(exc)}), 500
