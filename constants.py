...
LABEL_PATTERNS = [
    ("lieferdatum", re.compile(r"\b(lieferscheindatum|lieferdatum|liefertermin|tag der lieferung|tag d[\w.]*ief|lieferschein[\s-]?termin)\b", re.IGNORECASE)),
    ("belegdatum", re.compile(r"\b(beleg[-\s]?datum|beleg[-\s]?d[\w.]*|datum[-\s]?beleg)\b", re.IGNORECASE)),
    ("rechnungsdatum", re.compile(r"\b(rechn\.?-?\s*dat\.?|rechnungsdatum|rechnung[s-]?dat|rechn\s*d[\w.]*)\b", re.IGNORECASE)),
    ("druckdatum", re.compile(r"\b(druckdatum(?:\s*[/\-]?\s*zeit)?|druckdatum/-zeit|druck[-\s]?dat|druck[-\s]?d[\w.]*)\b", re.IGNORECASE)),
