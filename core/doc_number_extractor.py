...
class DocNumberExtractor:
    ...
    def extract_doc_number(
        self, text: str, supplier_canonical: Optional[str] = None, doc_type: Optional[str] = None
    ) -> DocNumberResult:
        """
    ...
    def _extract_supplier_specific(
        self, text: str, supplier_canonical: str, doc_type: Optional[str]
    ) -> DocNumberResult:
    ...
    def _extract_fallback(self, text: str, doc_type: Optional[str]) -> DocNumberResult:
    ...
    def _search_fields(self, text: str, field_names: List[str]) -> DocNumberResult:
        """
        ...
        for field in field_names:
            ...
            for idx, line in enumerate(lines):
                ...
                # darunter "325708  396901  30.03.26". In dem Fall muss der
                # Wert unter der Spalte genommen werden, nicht die erste Zahl.
                for j in range(idx + 1, min(idx + 3, len(lines))):
                    number = self._extract_aligned_number_from_table_row(line, lines[j], field)
                    if number:
    ...
    def _extract_aligned_number_from_table_row(self, header_line: str, value_line: str, field: str) -> Optional[str]:
        """Extrahiert einen Wert anhand der horizontalen Position unter einem Tabellenkopf."""
        if not header_line or not value_line or not field:
            return None

        field_pos = header_line.lower().find(field.lower())
        if field_pos < 0:
            return None

        candidates: List[Tuple[int, str]] = []
        for pattern in self._DOC_NUMBER_PATTERNS:
            for match in pattern.finditer(value_line):
                candidate = self._clean_number(match.group(1).strip())
                if self._is_plausible_doc_number(candidate):
                    candidates.append((match.start(1), candidate))

        # Nur Tabellenzeilen mit mehreren Werten brauchen Spaltenausrichtung.
        unique_candidates: List[Tuple[int, str]] = []
        seen = set()
        for start, candidate in sorted(candidates, key=lambda item: item[0]):
            key = (start, candidate)
            if key not in seen:
                unique_candidates.append((start, candidate))
                seen.add(key)

        if len(unique_candidates) < 2:
            return None

        best_start, best_candidate = min(
            unique_candidates,
            key=lambda item: abs(item[0] - field_pos),
        )

        # Wenn der naheliegende Wert weit links vom Feldkopf steht, ist die
        # Formatierung wahrscheinlich nicht spaltenartig genug.
        if best_start + max(len(best_candidate), 1) < field_pos:
            return None
...
def extract_doc_number(
    text: str, supplier_canonical: Optional[str] = None, doc_type: Optional[str] = None
) -> DocNumberResult:
    """
