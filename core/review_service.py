...
def decide_review_status(extraction_result: Dict, settings: ReviewSettings) -> ReviewDecision:
    """
    ...
    # Entscheidung
    if reasons:
        return ReviewDecision(status=DocumentStatus.NEEDS_REVIEW, reasons=reasons, details=details)
    else:
        return ReviewDecision(status=DocumentStatus.READY, reasons=[], details=details)
...
def build_final_filename(supplier_canonical: str, date_iso: str, doc_number: str, original_filename: str = "") -> str:
    """
...
def _make_unique_filename(target_dir: Path, filename: str) -> Path:
    """Macht Dateinamen eindeutig mit _01, _02, etc. Fallback auf UUID-Suffix."""
    import uuid
    ...
    # Fallback: UUID-Suffix um Kollision sicher zu vermeiden
    unique = target_dir / f"{stem}_{uuid.uuid4().hex[:8]}{suffix}"
    return unique


def finalize_document(
    doc_id: str, source_path: Path, extraction_result: Dict, settings: ReviewSettings, mode: str = "move"
) -> FinalizeResult:
    """
    ...
    try:
        ...
        return FinalizeResult(
            success=False, finalized_path=None, final_filename="", autosort_reason="", name_reason="", error=str(exc)
        )


def save_correction(corrections_path: Path, doc_id: str, user_email: str, original: Dict, corrected: Dict) -> None:
    """
    Speichert Korrektur für Audit + ML Training (JSONL append).

    Args:
        corrections_path: Path zu corrections.jsonl
        doc_id: Dokument-ID
...
def save_ground_truth_sample(ground_truth_path: Path, doc_id: str, ocr_text: str, corrected_labels: Dict) -> None:
    """
