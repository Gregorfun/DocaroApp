...
class TestDocNumberExtraction(unittest.TestCase):
    ...
    def test_manitowoc_lieferschein_auslieferung(self):
        """Test: Manitowoc Lieferschein nutzt die Nummer neben Auslieferung."""
        text = """
        Manitowoc
        Packing Slip
        MTW        1001341981

        Datum / Zeit    31.03.2026 / 17:46
        Sendung:
        Auslieferung:   801278377
        Incoterm:       DAP
        """
        result = self.extractor.extract_doc_number(text, "Manitowoc", "Lieferschein")

        self.assertIsNotNone(result.doc_number)
        self.assertEqual(result.doc_number, "801278377")
        self.assertEqual(result.confidence, "high")
        self.assertEqual(result.source_field, "Auslieferung")

    def test_herbrand_l_schein_from_table_header(self):
        """Test: Herbrand L-Schein ist die Lieferscheinnummer, nicht KD-NR."""
        text = """
        Herbrand Fichtenhain
        GmbH & Co. KG

        KD-NR.      L-Schein      Datum      Blatt
        325708      396901        30.03.26   1
        Lieferdatum: 19.03.26
        """
        result = self.extractor.extract_doc_number(text, "Herbrand Fichtenhain", "Lieferschein")

        self.assertIsNotNone(result.doc_number)
        self.assertEqual(result.doc_number, "396901")
        self.assertEqual(result.confidence, "high")
        self.assertEqual(result.source_field, "L-Schein")

    def test_herbrand_l_schein_next_line(self):
        """Test: Herbrand L-Schein funktioniert auch bei Zeilenumbruch direkt darunter."""
        text = """
        Herbrand Fichtenhain
        L-Schein
        396901
