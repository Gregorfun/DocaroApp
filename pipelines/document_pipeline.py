    ...
    **Workflow**:
    ...
    def process_document(self, pdf_path: Path, config: Optional[Dict] = None) -> PipelineResult:
        ...
        try:
            ...
            _LOGGER.info(
                f"   Qualität: OCR nötig={quality['needs_ocr']}, " f"Text-Coverage={quality['text_coverage']:.2f}"
            )
            ...
            if quality["needs_ocr"]:
                ...
                ocr_result = process_with_ocr(pdf_path, method=config.get("ocr_method", "auto") if config else "auto")
                ...
                if ocr_result.success:
                    ...
                    _LOGGER.info(f"   OCR erfolgreich mit {ocr_method} " f"(Conf: {ocr_result.confidence:.2f})")
                else:
            ...
            docling_result = docling_processor.process(
                processed_pdf, extract_tables=True, extract_layout=True, chunk_text=True
            )
            ...
            _LOGGER.info(
                f"   Docling: {len(docling_result.tables)} Tabellen, "
            ...
            if needs_review:
                ...
                qm = QuarantineManager(
                    quarantine_dir=Config.QUARANTINE_DIR, quarantine_log=Config.DATA_DIR / "quarantine.jsonl"
                )
